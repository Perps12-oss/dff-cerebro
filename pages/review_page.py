...
def build_cleanup_deletion_groups(
    groups: List[GroupData],
    keep_states: Dict[int, Dict[str, bool]],
) -> Tuple[List[Dict[str, Any]], int]:
    """Build strict DeletionPlan groups from ReviewPage keep/delete state."""
    ...
    for g in groups:
        ...
        keeper = next((p for p in g.paths if keep_map.get(p, True)), "")
        if not keeper:
            # UI should prevent this; never guess a keeper for destructive work.
            continue
...
class ReviewPage(BaseStation):
    ...
    def _open_ceremony(self):
        delete_groups, total_delete_size = build_cleanup_deletion_groups(
            self._filtered_groups,
            self._keep_states,
        )
