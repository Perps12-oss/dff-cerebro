...
class _HashCache:
    ...
    Internally it uses HashCache (mtime in ns + dev/inode).  User-facing
    duplicate groups must be exact, so this adapter intentionally reads and
    writes the full-hash slot instead of the sampled quick-hash slot.
    """
...
class FastPipeline:
    ...
    def _quick_hash_with_meta(self, f: FastFileInfo) -> Tuple[FastFileInfo, Optional[str]]:
        path = f.path
        try:
            # This hash is the final duplicate key.  It must cover the entire
            # file; sampled hashes can mark different large files as duplicates.
            h = hashlib.md5()
